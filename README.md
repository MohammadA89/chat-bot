...
| همان پیام، با اشاره به `127.0.0.1` | تله‌ی localhost؛ پایین را ببینید |
...
### وقتی مرورگر اصلاً نمی‌تواند به سرویس وصل شود

بعضی endpointها از مرورگر قابل فراخوانی نیستند — نه به‌خاطر تنظیمات شما، بلکه به‌خاطر خودشان:

- هدر `Access-Control-Allow-Origin` نمی‌فرستند، یا
- در پاسخ preflight فقط `Access-Control-Allow-Headers: *` می‌دهند و اسم `Authorization` را نمی‌آورند. طبق استاندارد Fetch این ستاره عمداً شامل `Authorization` نمی‌شود، و هر درخواست این اپ این هدر را دارد.

هیچ تنظیمی در سمت کلاینت این را حل نمی‌کند. نشانه‌ی گویا این است که `curl` همان endpoint را بی‌نقص می‌گیرد — چون `curl` اصلاً preflight نمی‌فرستد.

**راه‌حل عمومی: API را از همان origin اپ سرو کنید.** این کار به هیچ سرویس خاصی وابسته نیست.

</div>

```bash
cp .env.example .env.local
# VITE_API_PROXY را برابر ریشه‌ی API خود بگذارید، مثلاً:
#   VITE_API_PROXY=http://127.0.0.1:8080
#   VITE_API_PROXY=https://api.example.com
npm run dev
```

<div dir="rtl">

حالا در صفحه‌ی اتصال دکمه‌ی «استفاده از proxy همین سرور» ظاهر می‌شود و Base URL را برایتان پر می‌کند. بدون `VITE_API_PROXY` نه دکمه‌ای هست و نه مسیر proxy‌ای ساخته می‌شود؛ رفتار اپ دقیقاً مثل قبل است.

**Base URL نسبی.** این قابلیت روی یک چیز عمومی‌تر بنا شده: هر Base URL که با `/` شروع شود از همان origin خوانده می‌شود. یعنی اگر اپ را پشت nginx، یک سرور خودتان یا هر reverse proxy دیگری بالا آورده‌اید، کافی است مسیر نسبی بدهید:
...
### تله‌ی localhost روی ویندوز

اگر سرویس مدل روی همین دستگاه اجرا می‌شود و با `localhost` کار نمی‌کند اما `curl` سالم جواب می‌دهد، تقریباً همیشه علتش این است:

```text
localhost  →  ::1  (IPv6)      ← مرورگر اول این را امتحان می‌کند
           →  127.0.0.1 (IPv4) ← سرویس فقط اینجا گوش می‌دهد
```

سرویسی که روی `0.0.0.0` یا `127.0.0.1` گوش می‌دهد فقط IPv4 است و از راه `::1` دیده نمی‌شود. مرورگر روی ویندوز اول IPv6 را می‌زند و شکست می‌خورد؛ `curl` به IPv4 برمی‌گردد و همه‌چیز را سالم نشان می‌دهد. خطای مرورگر شبیه CORS است ولی CORS نیست.

**راه‌حل:** در Base URL به‌جای `localhost` بنویسید `127.0.0.1` — مثلاً `http://127.0.0.1:8080/v1`. اپ هم وقتی Base URL روی `localhost` باشد و اتصال شکست بخورد، همین را در متن خطا یادآوری می‌کند.
