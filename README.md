...
### سرویس محلی و هدر Authorization

مرورگر قبل از هر درخواستِ cross-origin که هدر `Authorization` دارد، یک preflight می‌فرستد. اگر پاسخ preflight فقط `Access-Control-Allow-Headers: *` باشد و اسم `Authorization` را صریح نیاورد، مرورگر درخواست را **رد می‌کند** — طبق استاندارد Fetch، ستاره عمداً شامل `Authorization` نمی‌شود. این خطا از سمت کلاینت قابل دور زدن نیست و در اپ به‌شکل «اتصال به سرور برقرار نشد» دیده می‌شود، در حالی که `curl` همه‌چیز را سالم نشان می‌دهد.

راه‌حل: API را از همان origin اپ سرو کنید تا اصلاً cross-origin نماند.

</div>

```bash
cp .env.example .env.local     # و مقدار API_PROXY را ویرایش کنید
npm run dev
```

<div dir="rtl">

سپس در اپ این Base URL را بگذارید:

</div>

```text
http://127.0.0.1:5173/proxy-api/v1
```

<div dir="rtl">

بدون `API_PROXY` هیچ مسیر proxy ساخته نمی‌شود و رفتار اپ مثل قبل است.
...
**راه‌حل:** در Base URL به‌جای `localhost` بنویسید `127.0.0.1` — مثلاً `http://127.0.0.1:20128/v1`. اپ هم وقتی Base URL روی `localhost` باشد و اتصال شکست بخورد، همین را در متن خطا یادآوری می‌کند.
