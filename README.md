...
**اگر مدلی با وجود همه‌ی این‌ها فراخوانی ابزار را به‌شکل متن چاپ کند**، harness آن پاسخ را دور می‌ریزد، یک بار مدل را تصحیح می‌کند و در صورت تکرار، پیام کوتاهی نشان می‌دهد که کاری انجام نشده است. JSON تولیدشده در متن مدل هرگز parse یا اجرا نمی‌شود — تنها فراخوانی ساختاریافته‌ی خود provider به ابزار واقعی تبدیل می‌شود.
...
| ویرایش | مدل به‌جای اعمال تغییر، آن را توصیف کرده | harness خودش یک بار مدل را وادار به اعمال می‌کند؛ اگر باز هم توصیف کرد مدل قوی‌تری انتخاب کنید |
